...
def dashBoard_view_temp1(request):
    ...
    centres = [centre(item["Quarantine centres"], item["Ready to be used (unit)"]) for i, item in enumerate(sort_todo) if i < 3]
    d = data(date = date, units_in_use = sum([todo["Current unit in use"] for todo in todos]), units_available = sum([todo["Ready to be used (unit)"] for todo in todos]))
    context = {"data": d, "centre": centre}
